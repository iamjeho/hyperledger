...
async function submit(args, res) {
    try {
        ...
        const userIdentity = await wallet.get("appuser12");
        if (userIdentity) {
            console.log(
                'An identity for the user "appuser12" already exists in the wallet'
            );
            return;
        }
        ...
        await wallet.put("appuser12", cx509Identity);
        console.log(
            'Successfully registered and enrolled admin user "appuser12" and imported it into the wallet'
        );
                // Check to see if we've already enrolled the user.
        const identity_user = await wallet.get("appuser12");
        if (!identity_user) {
            console.log(
                'An identity for the user "appuser12" does not exist in the wallet'
            );
...
app.post('/vote', async(req, res)=>{
    ...
    submit([count, candidate, location, time])
